...
assert
  (isNull immutableConfig || isPath immutableConfig)
  || throw "configPath must be a path. Or not passed at all";

assert
  (isNull outOfStoreConfig || isString outOfStoreConfig)
...
let
  ...
  nvimConfig =
    if isPath immutableConfig then
      immutableConfig
    else
      runCommandLocal "kickstart-config-symlink" { } "ln -s ${lib.escapeShellArg outOfStoreConfig} $out";
  ...
  # Make a neovim derivation by calling the wrapper directly.
  # `wrapNeovimUnstable` accepts plugins/luaRcContent/wrapperArgs natively;
