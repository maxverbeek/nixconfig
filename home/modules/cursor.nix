    ...
    assertions = [
      {
        ...
        message = "one of modules.cursor.xcursor or modules.cursor.hyprcursor must be enabled";
      }
    ];

    home.packages = [ cfg.package ];

    home.file.".local/share/icons/${cfg.name}" =
      if cfg.hyprcursor then { source = "${cfg.package}/share/icons/${cfg.name}"; } else { };
